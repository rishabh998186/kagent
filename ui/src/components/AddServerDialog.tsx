...
export function AddServerDialog({
  ...
  const handleSubmit = () => {
    ...
    if (activeTab === "url" && !url.trim().match(/^[a-z]+:\/\//i)) {
      setError(
        "Please enter a valid URL with protocol (e.g., http:// or https://)",
      );
      return;
    }
        ...
        const mcpServer: MCPServer = {
          ...
          spec: {
            deployment: {
              ...
              cmd: cmd,
              args: args,
              ...(Object.keys(formatEnvVars()).length > 0 && {
                env: formatEnvVars(),
              }),
            },
            transportType: "stdio",
        ...
        <div className="flex-1 overflow-y-auto px-6">
          ...
          <div className="space-y-4">
            ...
            <Tabs
              ...
              value={activeTab}
              onValueChange={(v) => setActiveTab(v as "command" | "url")}
            >
              ...
              <TabsContent value="url" className="pt-4 space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      id="use-streamable-http"
                      checked={useStreamableHttp}
                      onCheckedChange={handleUseStreamableHttpChange}
                    />
                    <Label htmlFor="use-streamable-http">
                      Use Streamable HTTP
                    </Label>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    Use Streamable HTTP to connect to the MCP server, instead of
                    SSE
                  </p>
                </div>
                ...
                {!useStreamableHttp && (
                  <div className="space-y-2">
                    <Label htmlFor="sse-read-timeout">
                      SSE Read Timeout (e.g., 60s)
                    </Label>
                    <Input
                      id="sse-read-timeout"
                      type="string"
                  ...
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setTerminateOnClose(Boolean(checked))
                      }
                    />
                    <Label htmlFor="terminate-on-close">
                      Terminate Connection On Close
                    </Label>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    When enabled, the server will terminate connection when the
                    client disconnects
                  </p>
                </div>
              </TabsContent>
